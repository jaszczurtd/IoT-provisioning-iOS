...
XIO_HANDLE mqtt_transport_ws_io(const char* fqdn, const HTTP_PROXY_OPTIONS* proxy_info);
...
static int create_transport_io_object(PROV_TRANSPORT_MQTT_INFO* mqtt_info)
{
    ...
    if (mqtt_info->transport_io == NULL)
    {
        ...
        if ((mqtt_info->transport_io = mqtt_transport_ws_io(mqtt_info->hostname, transport_proxy)) == NULL)
        {
