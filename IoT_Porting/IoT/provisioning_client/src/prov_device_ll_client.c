...
static const char *x509_certificate_pointer = NULL;
static const char *x509_key_pointer = NULL;

void Prov_Device_LL_Set_x509_cert(const char *certificate) {
    x509_certificate_pointer = certificate;
}

void Prov_Device_LL_Set_x509_key(const char *key) {
    x509_key_pointer = key;
}
...
PROV_DEVICE_RESULT Prov_Device_LL_Register_Device(PROV_DEVICE_LL_HANDLE handle, PROV_DEVICE_CLIENT_REGISTER_DEVICE_CALLBACK register_callback, void* user_context, PROV_DEVICE_CLIENT_REGISTER_STATUS_CALLBACK reg_status_cb, void* status_ctx)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                char* x509_cert = (char*)x509_certificate_pointer;
                char* x509_private_key = (char*)x509_key_pointer;
                
                if(x509_cert != NULL && x509_private_key != NULL) {
